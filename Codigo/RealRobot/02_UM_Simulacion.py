...
def calculate_best_action(world_model, utility_model, obs_t, input_mean_wm, input_std_wm, output_mean_wm, output_std_wm,
    ...
    best_action, obs_t1, best_utility = max(predicted_utilities, key=lambda x: x[2])

    print(f"\n  Mejor acción seleccionada: {best_action} (Utilidad: {best_utility:.4f})")
    
    return best_action, obs_t1
...
if __name__ == "__main__":
    ...
    utility_model = keras.models.load_model("utility_model_50_2.keras")
    ...
    actual_pos = [[0.6, 0, 0.6], [0.6, 0, 0.6], [0.6, 0, 0.6]]
    objetive_pos = [0, 0, 0]
