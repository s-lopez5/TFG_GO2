"""
###############TO DO#################
-Modificar funciones de recibir frames para que pille labeled markers.
-Evitar que el hilo de conexion printee todo el rato.
...
# Variables globales para almacenar los datos del último frame
last_frame_data = {
    'frame_number': 0,
    'markers': [],
    'rigid_bodies': [],
...
def receive_new_frame(data_dict):
    ...
    # Almacenar datos del frame actual
    last_frame_data['frame_number'] = data_dict.get('frameNumber', 0)
    last_frame_data['timestamp'] = data_dict.get('timestamp', 0)
...
def receive_new_frame_with_data(data_dict):
    ...
    # Almacenar datos del frame con información de mocap
    last_frame_data['frame_number'] = data_dict.get('frameNumber', 0)
    last_frame_data['timestamp'] = data_dict.get('timestamp', 0)
    
    # Extraer datos de marcadores si están disponibles
    if 'mocap_data' in data_dict and data_dict['mocap_data']:
        mocap_data = data_dict['mocap_data']
        
        # Almacenar marcadores no etiquetados
        if hasattr(mocap_data, 'unlabeled_marker_list'):
            last_frame_data['markers'] = []
            for marker in mocap_data.unlabeled_marker_list:
                if hasattr(marker, 'pos'):
                    last_frame_data['markers'].append({
                        'x': marker.pos[0],
                        'y': marker.pos[1],
                        'z': marker.pos[2]
                    })
        
        # Almacenar cuerpos rígidos
        if hasattr(mocap_data, 'rigid_body_list'):
            last_frame_data['rigid_bodies'] = []
            for rb in mocap_data.rigid_body_list:
                if hasattr(rb, 'pos') and hasattr(rb, 'rot'):
                    last_frame_data['rigid_bodies'].append({
                        'id': rb.id_num if hasattr(rb, 'id_num') else 'unknown',
                        'position': {
                            'x': rb.pos[0],
                            'y': rb.pos[1],
                            'z': rb.pos[2]
...
def print_marker_positions():
    """Imprime las posiciones de los marcadores del último frame recibido"""
    global last_frame_data
    
    print("\n" + "="*60)
    print(f"POSICIONES DEL ÚLTIMO FRAME RECIBIDO")
    print(f"Frame Number: {last_frame_data['frame_number']}")
    print(f"Timestamp: {last_frame_data['timestamp']}")
    print("="*60)
    
    # Imprimir marcadores no etiquetados
    if last_frame_data['markers']:
        print(f"\nMARCADORES NO ETIQUETADOS ({len(last_frame_data['markers'])} encontrados):")
        for i, marker in enumerate(last_frame_data['markers']):
            print(f"  Marcador {i+1}: X={marker['x']:.4f}, Y={marker['y']:.4f}, Z={marker['z']:.4f}")
    else:
        print("\nMARCADORES NO ETIQUETADOS: Ninguno encontrado")
    
    # Imprimir cuerpos rígidos
    if last_frame_data['rigid_bodies']:
        print(f"\nCUERPOS RÍGIDOS ({len(last_frame_data['rigid_bodies'])} encontrados):")
        for rb in last_frame_data['rigid_bodies']:
            pos = rb['position']
            rot = rb['rotation']
            print(f"  Cuerpo Rígido ID {rb['id']}:")
            print(f"    Posición: X={pos['x']:.4f}, Y={pos['y']:.4f}, Z={pos['z']:.4f}")
            print(f"    Rotación: X={rot['x']:.4f}, Y={rot['y']:.4f}, Z={rot['z']:.4f}, W={rot['w']:.4f}")
    else:
        print("\nCUERPOS RÍGIDOS: Ninguno encontrado")
    
    # Mostrar información específica para los primeros dos marcadores/objetos
    total_objects = len(last_frame_data['markers']) + len(last_frame_data['rigid_bodies'])
    if total_objects >= 2:
        print(f"\nRESUMEN - PRIMEROS DOS OBJETOS DETECTADOS:")
        object_count = 0
        
        # Primero los marcadores
        for i, marker in enumerate(last_frame_data['markers']):
            if object_count < 2:
                print(f"  Objeto {object_count + 1} (Marcador {i+1}): X={marker['x']:.4f}, Y={marker['y']:.4f}, Z={marker['z']:.4f}")
                object_count += 1
        
        # Luego los cuerpos rígidos
        for rb in last_frame_data['rigid_bodies']:
            if object_count < 2:
                pos = rb['position']
                print(f"  Objeto {object_count + 1} (Cuerpo Rígido ID {rb['id']}): X={pos['x']:.4f}, Y={pos['y']:.4f}, Z={pos['z']:.4f}")
                object_count += 1
    elif total_objects == 1:
        print(f"\nAVISO: Solo se encontró 1 objeto. Se necesitan al menos 2 marcadores.")
    else:
        print(f"\nAVISO: No se encontraron marcadores ni cuerpos rígidos.")
...
def print_commands(can_change_bitstream):
    ...
    outstring += "Marker Position Display\n"
    outstring += "  m  show positions of markers from last received frame\n"
    outstring += "General\n"
